/**
 * Cerebrum — interactive optical logo.
 * Uses the exact paths from the application's original logo.
 * No additional packages, downloaded assets, or tracking.
 */
...
export function staticFieldCss(accent, deep, { core = true } = {}) {
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
      <defs>
        <linearGradient id="a" x2="1" y2="1">
          <stop stop-color="${a}"/>
          <stop offset=".48" stop-color="#e3f7f1"/>
          <stop offset="1" stop-color="${a}"/>
        </linearGradient>
      </defs>
      <g fill="none" stroke="url(#a)" stroke-width=".65"
         stroke-linejoin="round" stroke-linecap="round">
        ${LOGO.map((p) => `<path d="${p}"/>`).join("")}
      </g>
    </svg>
  `;
  ...
  return [
    core
      ...
      : "",
    `radial-gradient(ellipse at 83% 56%, ${rgba(a, 0.09)}, transparent 48%)`,
    `linear-gradient(${d}, ${d})`,
  ].filter(Boolean).join(", ");
}
...
export async function createField(canvas, initial = {}) {
  ...
  function layout() {
    ...
    return {
      x: width * (narrow ? 0.78 : 0.8),
      y: height * (narrow ? 0.27 : 0.49),
      size: reading
        ? Math.min(width * 0.14, 190)
        : Math.min(
            narrow ? width * 0.48 : width * 0.32,
            height * 0.59,
            540
          ),
      strength: reading ? 0.28 : narrow ? 0.46 : 1,
    };
  }
  ...
  function draw() {
    ...
    // Quiet light behind the sculpture.
    const pool = ctx.createRadialGradient(
      cx, cy, 0,
      cx, cy, Math.max(scale * 1.35, 1)
    );
    pool.addColorStop(0, rgba(a, light ? 0.06 : 0.075));
    ...
    ctx.save();
    ctx.translate(cx, cy);
    ctx.rotate(-0.35 + Math.sin(time * 0.08) * 0.04);

    for (let i = 0; i < 3; i++) {
      const rx = scale * (1 + i * 0.17);
      const ry = scale * (0.37 + i * 0.045);
      const arc = ctx.createLinearGradient(-rx, 0, rx, 0);

      arc.addColorStop(0, rgba(a, 0));
      arc.addColorStop(0.55, rgba(a, 0.025 * l.strength));
      arc.addColorStop(0.85, rgba(a, 0.19 * l.strength));
      arc.addColorStop(1, rgba(a, 0));

      ctx.strokeStyle = arc;
      ctx.lineWidth = i === 0 ? 1.2 : 0.6;
      ctx.beginPath();
      ctx.ellipse(0, 0, rx, ry, 0, -0.85, 2.2);
      ctx.stroke();
    }
    ctx.restore();

    // Bounded tilt preserves the recognizable logo.
    const yaw = Math.sin(time * 0.17) * 0.14 + px * 0.27;
    const pitch = Math.cos(time * 0.13) * 0.07 + py * 0.12;
    const roll = -0.1 + Math.sin(time * 0.11) * 0.035 + px * 0.025;
    const unit = scale / 24;
    const depth = unit * (1 + pulse * 0.55);
    const strength =
      l.strength * clamp(Number(state.core) || 0, 0, 1);

    ctx.save();
    ctx.translate(cx, cy + Math.sin(time * 0.35) * 5);
    ctx.rotate(roll);
    ctx.transform(
      Math.cos(yaw),
      Math.sin(pitch) * 0.22,
    ...
    // Layered edges create a shallow extrusion.
    for (let i = 12; i >= 0; i--) {
      ctx.save();
      ctx.translate(
        (i / 12) * depth * (0.65 + yaw),
        (i / 12) * depth * (0.34 + pitch)
      );
      ctx.scale(unit, unit);
      ctx.translate(-12, -12);
      ctx.lineJoin = "round";
    ...
    // Reflective front face.
    const face = ctx.createLinearGradient(5 + px * 2, 3, 19, 23);
    face.addColorStop(0, light ? "#183d32" : "#e2fff2");
    face.addColorStop(0.19, a);
    face.addColorStop(0.46, light ? "#285b4c" : "#39594d");
    face.addColorStop(0.6, light ? a : "#ddfff2");
    ...
    // Moving reflection.
    ctx.save();
    const sweep = 12 + Math.sin(time * 0.31 + px * 0.6) * 13;
    ctx.beginPath();
    ...
    sheen.addColorStop(
      0.5,
      `rgba(240,255,250,${light ? 0.4 : 0.85})`
    );
    ...
    // Small glints travel along the original outline.
    ctx.setLineDash([0.7, 10, 0.2, 19]);
    ctx.lineDashOffset = -time * (1.2 + energy * 0.65);
    ctx.lineWidth = 0.13;
    ...
    // Protect the text on the left.
    const guard = ctx.createLinearGradient(0, 0, width, 0);
    guard.addColorStop(0, rgba(state.deep, 0.94));
    guard.addColorStop(0.4, rgba(state.deep, 0.68));
    guard.addColorStop(0.66, rgba(state.deep, 0));
    ctx.fillStyle = guard;
    ctx.fillRect(0, 0, width, height);
  }
  ...
  function tap(e) {
    ...
    if (Math.hypot(e.clientX - cx, e.clientY - cy) < scale * 0.48) {
      pulse = 1;
    }
  }
  ...
  return {
    setState(next = {}) {
      ...
      for (const key of [
        "mode", "energy", "core", "coreScale", "light", "paused"
      ]) {
